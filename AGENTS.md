...
## Production Merging (HUMAN-ONLY ACCESS)
- AI agents are **STRICTLY FORBIDDEN** from merging changes into Production (`POST /api/ai/merge-staging` is blocked for AI agents).
- Merging into production is exclusively reserved for the human user.
- After creating and testing changes in the `[STAGING]` branch, you must instruct the human user to open the Web UI (Diff Viewer), inspect the visual diffs, and click "Confirm Merge".
...
- **STANDARD ENDPOINTS**: Always use the official `/api/ai/*` endpoints for all task operations:
  ...
  - *(Note: `POST /api/ai/merge-staging` is restricted to human users via the Web UI)*.
