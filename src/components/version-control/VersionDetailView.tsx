...
export function VersionDetailView({
  ...
  const tasksBefore = useMemo(() => version?.stateBefore?.tasks || [], [version]);
  const tasksAfter = useMemo(() => version?.stateAfter?.tasks || [], [version]);
  const projectsBefore = useMemo(() => version?.stateBefore?.projects || [], [version]);
  ...
  const addedTasks = useMemo(() => tasksAfter.filter((t) => !beforeTaskIds.has(t.id)), [tasksAfter, beforeTaskIds]);
  const removedTasks = useMemo(() => tasksBefore.filter((t) => !afterTaskIds.has(t.id)), [tasksBefore, afterTaskIds]);

  const modifiedTaskDiffs = useMemo(() => {
    const list: Array<{
      task: SqlTask;
      oldTask: SqlTask;
      oldCode: string;
    ...
    return list;
  }, [tasksBefore, tasksAfter, beforeTaskIds]);
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-black overflow-y-auto min-w-0">
      {/* Top Sticky Header */}
      <div className="px-3 sm:px-5 py-2.5 sm:py-3 border-b border-slate-200/80 dark:border-zinc-800/80 bg-white/95 dark:bg-[#0c0c0e]/95 sticky top-0 z-20 backdrop-blur-md">
        <div className="flex items-center justify-between gap-2 sm:gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {onBack && (
              <button
                type="button"
                onClick={onBack}
                className="p-1 -ml-1.5 text-slate-500 hover:text-slate-800 dark:text-zinc-400 dark:hover:text-zinc-100 transition-colors cursor-pointer shrink-0 rounded-md"
            ...
            <div className="flex items-center gap-1.5 flex-wrap min-w-0">
              <span
                className={cn(
                  "text-[9.5px] sm:text-[10px] font-bold tracking-wider px-2 py-0.5 rounded-md border shrink-0 flex items-center gap-1.5 whitespace-nowrap",
                  badge.className
                )}
              >
                {badge.icon}
              ...
              {isProjectDeleted && (
                <span className="text-[8.5px] sm:text-[9px] font-semibold px-1.5 py-0.5 rounded text-zinc-500 bg-zinc-500/15 border border-zinc-500/20 shrink-0 whitespace-nowrap">
                  Project Deleted
                </span>
              )}

              {version.isUndone ? (
                <span className="text-[9px] sm:text-[9.5px] font-bold tracking-wider px-1.5 py-0.5 rounded text-amber-700 dark:text-amber-300 bg-amber-500/15 border border-amber-500/30 shrink-0 whitespace-nowrap">
                  Undone
                </span>
              ) : (
                <span className="text-[9px] sm:text-[9.5px] font-bold tracking-wider px-1.5 py-0.5 rounded text-emerald-700 dark:text-emerald-300 bg-emerald-500/15 border border-emerald-500/30 shrink-0 whitespace-nowrap">
                  Active
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 shrink-0 ml-auto">
            <div className="text-[10px] sm:text-[11px] text-slate-400 dark:text-zinc-500 font-mono shrink-0 whitespace-nowrap">
            ...
            <div className="hidden md:flex items-center gap-1.5 shrink-0">
              ...
              >
                ...
                <span>
                  {version.action === "delete_project" ? "Resurrect" : "Undo Snapshot"}
                </span>
              </button>
      ...
      <div className="flex-1 p-3.5 sm:p-5 space-y-3 sm:space-y-4 overflow-y-auto">
        ...
        <div className="pb-3 border-b border-slate-100 dark:border-zinc-800/60">
          ...
          <p className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-zinc-100 leading-relaxed">
            {version.description || "Database snapshot recorded"}
          </p>
        </div>

        {/* Metrics & State Transition - Containerless */}
        <div className="pb-3 border-b border-slate-100 dark:border-zinc-800/60 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2 text-slate-600 dark:text-zinc-300 flex-wrap">
          ...
          <div className="flex items-center gap-2 text-[10.5px] text-slate-500 dark:text-zinc-400 font-mono flex-wrap">
            <span>{projectsAfter.length} projects</span>
            <span>•</span>
            <span className="flex items-center gap-1.5">
              ...
              {modifiedTaskDiffs.length > 0 && (
                <span className="text-sky-600 dark:text-sky-400 font-semibold">
                  ~{modifiedTaskDiffs.length} modified
                </span>
              )}
              {removedTasks.length > 0 && (
        ...
        {/* Primary View Switcher: Changes Diff (Old vs New) vs Full Snapshot */}
        <div className="pt-1 sm:pt-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-slate-200/80 dark:border-zinc-800/80 pb-2.5">
            {/* 2-Tab Segmented Control */}
            <div className="grid grid-cols-2 sm:flex items-center bg-slate-100 dark:bg-zinc-900 p-0.5 rounded-lg text-xs font-semibold w-full sm:w-auto">
              <button
                type="button"
                onClick={() => setActiveViewTab("diff")}
                className={cn(
                  "px-3 py-1.5 rounded-md flex items-center justify-center gap-1.5 transition-all cursor-pointer whitespace-nowrap text-xs min-h-[34px]",
                  activeViewTab === "diff"
                    ? "bg-white dark:bg-zinc-800 text-blue-600 dark:text-blue-400 shadow-2xs font-bold"
                    : "text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200"
                )}
              >
                <DiffIcon size={13} className="shrink-0" />
            ...
            {activeViewTab === "snapshot" && (
              <div className="flex items-center gap-2 justify-between sm:justify-end w-full sm:w-auto">
                {/* Type Filter Pills */}
                <div className="flex items-center bg-slate-100 dark:bg-zinc-800 rounded-lg p-0.5 text-[10.5px]">
                  {(["all", "sql", "edge"] as const).map((tab) => (
                    <button
                      key={tab}
                      onClick={() => setSelectedTaskTab(tab)}
                      className={cn(
                        "px-2.5 py-1 sm:py-0.5 rounded-md font-medium capitalize transition-all cursor-pointer",
                        selectedTaskTab === tab
                ...
                <div className="relative flex-1 sm:flex-initial">
                  ...
                  <input
                    ...
                    className="w-full sm:w-36 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-md pl-7 pr-2 py-1 sm:py-0.5 text-[11px] text-slate-800 dark:text-zinc-200 outline-none focus:border-blue-500"
                  />
                </div>
              </div>
            )}
          ...
          {/* TAB 1: ACTUAL CHANGES DIFF (OLD vs NEW) - Container-less Layout */}
          {activeViewTab === "diff" && (
