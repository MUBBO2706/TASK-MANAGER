...
interface VersionControlSkeletonProps {
  sidebarWidth?: number;
  ...
  return (
    <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-zinc-800/60 p-0">
      {[1, 2, 3, 4, 5, 6, 7].map((item) => (
        ...
        >
          {/* Top row: badge + time + stats before chevron - Strictly single row */}
...
export const VersionDetailSkeleton = () => {
  return (
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-black overflow-y-auto">
      ...
      <div className="px-4 py-3 sm:px-5 sm:py-3.5 border-b border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-[#0c0c0e]">
        <div className="flex items-center gap-2">
          <div className="h-5 w-5 text-slate-400 dark:text-zinc-600 md:hidden animate-pulse" />
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <div className="h-4 w-20 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
              <div className="h-4 w-14 bg-slate-100 dark:bg-zinc-800/60 rounded animate-pulse" />
            </div>
            <div className="h-3 w-36 bg-slate-100 dark:bg-zinc-800/60 rounded animate-pulse" />
          </div>
        </div>
      </div>

      {/* Container-less Body Skeleton */}
      <div className="flex-1 p-4 sm:p-5 space-y-3.5 overflow-y-auto">
        {/* Scope inline row */}
        <div className="flex items-center justify-between gap-4 pb-3 border-b border-slate-100 dark:border-zinc-800/60">
        ...
        {/* Description line */}
        <div className="pb-3 border-b border-slate-100 dark:border-zinc-800/60 space-y-1.5">
          <div className="h-2.5 w-24 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
          <div className="h-4 w-3/4 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
        </div>

        {/* Metrics line */}
        <div className="flex items-center justify-between gap-4 pb-3 border-b border-slate-100 dark:border-zinc-800/60">
          <div className="h-3.5 w-44 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
          <div className="h-3.5 w-36 bg-slate-100 dark:bg-zinc-800/60 rounded animate-pulse" />
        </div>

        {/* Compact notice */}
        <div className="h-8 rounded-lg bg-blue-50/40 dark:bg-blue-950/20 animate-pulse" />

        {/* Container-less tasks list */}
        <div className="pt-2">
          <div className="flex items-center justify-between pb-2.5">
            <div className="h-3.5 w-32 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
            <div className="h-6 w-40 bg-slate-100 dark:bg-zinc-800/60 rounded animate-pulse" />
          </div>
          <div className="divide-y divide-slate-100 dark:divide-zinc-800/60 border-t border-b border-slate-200/80 dark:border-zinc-800/70">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="py-2.5 px-1 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <div className="h-3.5 w-3.5 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
                  <div className="h-3.5 w-44 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
                </div>
                <div className="h-3.5 w-12 bg-slate-100 dark:bg-zinc-800/60 rounded animate-pulse" />
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Fixed Bottom Footer Skeleton for Undo / Redo */}
      <div className="sticky bottom-0 left-0 right-0 z-20 px-4 py-2.5 sm:px-5 sm:py-3 border-t border-slate-200 dark:border-zinc-800/80 bg-white/95 dark:bg-[#0c0c0e]/95 backdrop-blur-md flex items-center justify-between gap-3 shrink-0">
        <div className="h-3.5 w-36 bg-slate-100 dark:bg-zinc-800/60 rounded animate-pulse hidden sm:block" />
        <div className="flex items-center gap-2.5 w-full sm:w-auto">
          <div className="flex-1 sm:w-32 h-8 rounded-lg bg-slate-200 dark:bg-zinc-800 animate-pulse" />
          <div className="flex-1 sm:w-32 h-8 rounded-lg bg-slate-200 dark:bg-zinc-800 animate-pulse" />
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div className="h-12 px-3 sm:px-4 border-b border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-[#0c0c0e] flex items-center justify-between gap-3 shrink-0">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <div className="p-1.5 -ml-1 text-slate-400 dark:text-zinc-600">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400 dark:text-zinc-500">
            ...
            <div className="h-4 w-52 bg-slate-200 dark:bg-zinc-800 rounded animate-pulse" />
            <div className="h-4 w-20 bg-blue-500/10 rounded-full animate-pulse hidden sm:block ml-1" />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex w-full h-full overflow-hidden min-h-0 relative">
        ...
        >
          {/* Subheader: Filter / Search line matching VersionControlPage */}
        ...
        {/* Right Details Pane Skeleton */}
        <div className="hidden md:flex flex-1 flex-col h-full bg-slate-50 dark:bg-black min-w-0">
